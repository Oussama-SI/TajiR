          ...
          <div>
            <?php if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') ==3): ?>
              ...
              <div class="dropdown">
                ...
                <span class="mx-2"><img src="<?= validate_image($_settings->userdata('avatar')) ?>" class="img-thumbnail rounded-circle" alt="User Avatar" id="client-img-avatar">  <span class="mx-2"> <?= !empty($_settings->userdata('username')) ? $_settings->userdata('username') : $_settings->userdata('email') ?></span>
                </a>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  <a class="dropdown-item" href="./?page=manage_account">Gestion de compte</a>
                  <a class="dropdown-item" href="<?= base_url.'classes/Login.php?f=logout_client' ?>">Se déconnecter</a>
                </div>
              </div>
            <?php else: ?>
             Vous étes : <a href="./login.php" class="mx-2 text-light text-decoration-none font-weight-bolder">Client </a> | 
              <a href="./vendor" class="mx-2 text-light text-decoration-none font-weight-bolder">Vendeur </a>
            <!--  <a href="./admin" class="mx-2 text-light text-decoration-none font-weight-bolder">Admine Login</a>-->
            <?php endif; ?>
          </div>
        </div>
      </nav>
        ...
        <div class="container">
          ...
          <div class="collapse navbar-collapse order-3" id="navbarCollapse">
            ...
            <ul class="navbar-nav">
              ...
              <li class="nav-item">
                <a href="./?page=products" class="nav-link <?= isset($page) && $page =='products' ? "active" : "" ?>">Produits</a>
              </li>
              <li class="nav-item">
                <a href="./?page=about" class="nav-link <?= isset($page) && $page =='about' ? "active" : "" ?>">À propos de nous</a>
              </li>
              <?php if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 3): ?>
              <li class="nav-item">
                <?php 
                ...
                ?>
                <a href="./?page=orders/cart" class="nav-link <?= isset($page) && $page =='orders/cart' ? "active" : "" ?>"><span class="badge badge-secondary rounded-cirlce"><?= format_num($cart_count) ?></span> Carte</a>
              </li>
              <li class="nav-item">
                <a href="./?page=orders/my_orders" class="nav-link <?= isset($page) && $page =='orders/my_orders' ? "active" : "" ?>">Mes commands</a>
              </li>
              <?php endif; ?>
            </ul>
          </div>
